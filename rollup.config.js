...
const out = "docs";
try {
  fs.rmSync(join(process.cwd(), out, "build"), { recursive: true });
...
const generateSW = () => ({
    ...
    const essentialCache = new Set(["./", ...globSync(SSS.cache.files.essential, { nodir: true, cwd: join(process.cwd(), out) })]);
      ...
      globSync("**/*", { ignore: [...SSS.cache.files.ignore, ...SSS.cache.files.essential], nodir: true, cwd: join(process.cwd(), out) })
		...
		// Replace markers:
		let sw = `/* WARNING!!! THIS FILE WAS AUTO-GENERATED BY rollup.config.js AND WAS COPIED FROM "src/sw.template.js". ANY CHANGES TO THIS FILE WILL BE OVERWRITTEN ON THE NEXT BUILD!!! */\n\n` + template
...
export default {
    ...
    //   const indexOfNM = parts.indexOf("node_modules");
    //   const separatePackages = [
    //     "@pooltogether",
    //     "@web3modal",
    //     "@walletconnect",
    //     "@stablelib",
    ...
    //   ]
    //   if(indexOfNM > -1) {
    //     if(separatePackages.includes(parts[indexOfNM + 1])) return parts[indexOfNM + 1];
    //     else return "vendor";
    //   } else return "app";
    // }
	},
